  ...
  <main class="container py-4 pt-5 mt-5 min-vh-100">
    ...
    <header class="text-center mb-5">
      ...
      <p class="lead text-secondary">
        {{ authStore.esAdmin ? 'Gestiona tu información personal y credenciales de acceso.' : '¡Mira todo lo que has logrado hasta ahora!' }}
      </p>
    </header>

    <section v-if="authStore.esAdmin" aria-labelledby="admin-title" class="row justify-content-center">
      <div class="col-12 col-md-8 col-lg-6">
        <article class="card shadow-lg border-0 rounded-4">
          ...
          <div class="card-body p-4">
            <div v-if="adminStore.mensajeExito" class="alert alert-success text-center fw-bold shadow-sm">
              {{ adminStore.mensajeExito }}
            </div>
            <div v-if="adminStore.mensajeError" class="alert alert-danger text-center fw-bold shadow-sm">
            ...
            <form @submit.prevent="guardarCambiosAdmin">
              ...
              <div class="mb-3">
                <label for="nombreAdmin" class="form-label fw-bold text-secondary">Nombre Completo</label>
                <input type="text" id="nombreAdmin" v-model="formAdmin.nombre" class="form-control" required />
              </div>

              <div class="mb-4">
                <label for="usuarioAdmin" class="form-label fw-bold text-secondary">Usuario de Acceso</label>
                <input type="text" id="usuarioAdmin" v-model="formAdmin.usuario" class="form-control" required />
              </div>

              <hr class="my-4 text-muted">

              <h3 class="h6 fw-bold text-primary mb-3">🔒 Cambiar Contraseña (Opcional)</h3>
              <p class="small text-muted mb-3">Si no deseas cambiarla, deja estos campos en blanco.</p>

              <div class="row g-3 mb-4">
                <div class="col-md-6">
                  <label for="passAdmin" class="form-label fw-bold text-secondary small">Nueva Contraseña</label>
                  <input type="password" id="passAdmin" v-model="formAdmin.password" class="form-control" placeholder="Mínimo 6 caracteres" />
                </div>
                <div class="col-md-6">
                  <label for="passConfirm" class="form-label fw-bold text-secondary small">Confirmar Contraseña</label>
                  <input type="password" id="passConfirm" v-model="formAdmin.confirmPassword" class="form-control" placeholder="Repite la contraseña" />
                </div>
              </div>

              <button type="submit" class="btn btn-primary w-100 fw-bold fs-5 shadow-sm" :disabled="adminStore.cargando">
                <span v-if="adminStore.cargando" class="spinner-border spinner-border-sm me-2"></span>
    ...
    <section v-else-if="authStore.esChild" aria-labelledby="child-title" class="row justify-content-center">
      <div class="col-12 col-md-10 col-lg-8">
        <article class="card shadow-lg border-0 rounded-4 overflow-hidden">
          ...
          <header class="bg-info text-white py-5 text-center position-relative">
            <h2 id="child-title" class="display-4 fw-bold mb-0">¡Hola, {{ authStore.usuarioActual?.nombre }}! 🌟</h2>
          ...
          <div class="card-body p-5 bg-light">
            <div class="row g-4 text-center">
              ...
              <div class="col-12 col-sm-4">
                <div class="p-4 bg-white rounded-4 shadow-sm border border-2 border-primary h-100">
                  <div class="display-1 mb-2">🎓</div>
                  <h3 class="h5 fw-bold text-secondary mb-1">Nivel Actual</h3>
                  <p class="fs-4 fw-bold text-primary mb-0">Nivel {{ authStore.usuarioActual?.nivel || 1 }}</p>
                </div>
              </div>

              <div class="col-12 col-sm-4">
                <div class="p-4 bg-white rounded-4 shadow-sm border border-2 border-warning h-100">
                  <div class="display-1 mb-2">🏅</div>
                  <h3 class="h5 fw-bold text-secondary mb-1">Medallas</h3>
                  <p class="fs-4 fw-bold text-warning mb-0">{{ authStore.usuarioActual?.medallasCount || 0 }} Ganadas</p>
                </div>
              </div>

              <div class="col-12 col-sm-4">
                <div class="p-4 bg-white rounded-4 shadow-sm border border-2 border-success h-100">
                  <div class="display-1 mb-2">🎂</div>
                  <h3 class="h5 fw-bold text-secondary mb-1">Edad</h3>
                  <p class="fs-4 fw-bold text-success mb-0">{{ authStore.usuarioActual?.edad || '?' }} años</p>
                </div>
              </div>
            ...
            <div class="mt-5 text-center">
              ...
              <router-link :to="`/juego/nivel-${authStore.usuarioActual?.nivel || 1}`" class="btn btn-lg btn-success fw-bold px-5 shadow-sm rounded-pill">
                🎮 ¡Ir a jugar ahora!
              </router-link>
            </div>
